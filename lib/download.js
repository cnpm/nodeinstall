...
module.exports = function* download(options) {
  ...
  yield downloadTgz(tgzUrl, targetFile);
...
function* downloadTgz(tgzUrl, targetFile) {
  const res = yield request(tgzUrl, {
    timeout: 20000,
    streaming: true,
  });
...
function* verifyChecksum(shaUrl, targetFile) {
  const res = yield request(shaUrl, {
    type: 'text',
  });
