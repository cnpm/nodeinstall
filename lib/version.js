...
module.exports = {
  ...
  async getAlinodeVersion(options) {
    options = options || {};
    const distUrl = options.distUrl;
    assert(distUrl, 'distUrl is required');
    const versions = await getAlinodeVersions(distUrl);
    return _getNodeVersion(options.version, versions);
  },

  async getNodeVersion(options) {
    options = options || {};
    const distUrl = options.distUrl;
    assert(distUrl, 'distUrl is required');
    const versions = await getNodeVersions(distUrl);
    return _getNodeVersion(options.version, versions);
  },

  async getLocalNodeVersion(cwd) {
  ...
  async getNightlyVersion() {
    return await nodeNightlyVersion();
  },
...
async function getNodeVersions(distUrl) {
  const res = await request(distUrl + '/index.json', {
    method: 'GET',
    followRedirect: true,
    dataType: 'json',
    gzip: true,
...
async function getAlinodeVersions(distUrl) {
  const res = await request(distUrl + '/index.tab', {
    timeout: 10000,
    method: 'GET',
    followRedirect: true,
    dataType: 'text',
