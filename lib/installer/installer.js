...
class Installer extends EventEmitter {
  ...
  * install() {
    ...
    // use x64 when node version < 16
    if (arch === 'arm64' && !isVersionGreaterThanNode16(version)) {
      arch = 'x64';
    }
    arch = (arch !== 'x64' && arch !== 'arm64') ? 'x86' : arch;
    ...
    try {
      ...
      if (cacheStrategy.disable || !fs.existsSync(targetFile)) {
        this.emit('download', { tgzUrl, shaUrl, targetFile });
...
function isVersionGreaterThanNode16(version) {
