...
class Installer {
  ...
  * [installWindows](version) {
    ...
    const nodeDir = path.join(cwd, 'node_modules/node');
    const nodeLink = path.join(cwd, 'node_modules/.bin/node.exe');
    // const npmLink = path.join(cwd, 'node_modules/.bin/npm');
    const arch = os.arch() === 'x64' ? 'x64' : 'x86';
    const shaUrl = `${distUrl}/v${version}/SHASUMS256.txt`;
    const tgzUrl = `${distUrl}/v${version}/win-${arch}/node.exe`;

    console.log(1111);
    yield this[download](tgzUrl, shaUrl, nodeDir);
    if (!fs.existsSync(nodeLink)) {
      fs.symlinkSync(path.normalize('../node/bin/node.exe'), nodeLink);
    }
    // if (!fs.existsSync(npmLink)) {
    //   fs.symlinkSync(path.normalize('../node/bin/npm'), npmLink);
    // }
  ...
  * [download](srcUrl, shaUrl, targetDir) {
    ...
    try {
      ...
      switch (path.extname(targetFile)) {
        case '.zip':
          yield compressing.zip.uncompress(targetFile, targetDir, { strip: 1 });
          break;
        case '.tgz':
          yield extractTgz(targetFile, targetDir);
          break;
        case '.exe':
          yield fs.rename(targetFile, path.join(targetDir, path.basename(targetFile)));
          break;
        default:
      }
    } catch (err) {
...
function* verifyChecksum(shaUrl, targetFile) {
  ...
  let tgzName;
  if (/\.exe$/i.test(targetFile)) {
    // Windows file has a directory, for example, win-x86/node.exe
    tgzName = targetFile.split(path.sep).slice(-2).join(path.sep);
  } else {
    tgzName = path.basename(targetFile);
  }
